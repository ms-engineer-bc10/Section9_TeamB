...
def generate_images(story_pages, child, book_title):
    ...
    try:
        # 表紙の生成
        cover_prompt = f"""
        子ども向けの物語『{book_title}』に合わせた画像を作成してください。以下の指示を厳密に守ってください：
        1. {child.name}という名前の{child_age}歳の{child.gender}の子どもを中心に描いてください。
        2. 背景は柔らかく、親しみやすい雰囲気で、絵本らしいデザインにしてください。
        3. 画像全体が明るく、カラフルで、子どもの注目を引くようにしてください。
        4. キャラクターの表情は明るく、笑顔で描いてください。
        """
        ...
        for i, page_content in enumerate(story_pages, start=1):
            prompt = f"""
            子ども向けの物語の{i}のシーンに基づくカラフルで高品質なイラストをそれぞれ1枚ずつ描いてください。以下の指示を厳密に守ってください：
            1. このページの内容: {page_content}
               このストーリーの内容を視覚的に表現してください。ただし、{page_content}の文字情報は絶対に含めないでください。
            2. {main_character_prompt}
               このキャラクターを必ず含め、キャラクターの髪型や服装は常に同じにしてください。
            3. イラストには一切のテキスト、文字、記号を含めないでください。純粋にイラストのみで構成してください。
            4. 広い横長のフォーマットで描いてください。
            5. {style_prompt}
            6. 前のページのイラストと一貫性を持たせ、同じテイストやスタイルを維持してください。
            7. アルファベットや数字などの文字は一切表示しないでください。
            8. この指示自体をイラストに含めないでください。純粋にイラストのみを生成してください。
            9. ストーリーの進行に合わせて、場面や登場人物の動きを適切に変化させてください。
            10. 画像に、特定の地域や国の文化を反映するような描写や人々の服装を含まないでください。
            """
