...
def generate_story(child):
    ...
    try:
        response = client.chat.completions.create(
            model="gpt-4",
            messages=[
                {"role": "system", "content": "あなたは、出自を知る権利の専門知識を持つ日本で人気の絵本作家です。出自や養育背景が複雑な養育家庭における真実告知を目的として子ども向けの絵本を作成してください。真実告知として適切な内容であると同時に、直接的になりすぎないよう配慮して子ども向けの絵本らしく楽しめる内容にしてください。昔話ではないので、「むかしむかし」という表現は使わないでください。必ず日本語のひらがなとカナカナのみで、漢字は使用せず、8ページの物語を作成してください。各ページにタイトルをつけず、ストーリーのみを書いてください。何ページ目かの数字の記載も入れないでください。"},
...
def generate_book_title(child):
    prompt = f"{child.name}の家族についての子供向け絵本のタイトルを作成してください。この本は{child.background_type}について直接的になりすぎない表現で、優しく扱います。ネガティブな単語は使わないでください。"
